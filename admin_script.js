...
function showSubjectsView(className) {
    currentClass = `${className}th`; // Match student app format like "1st", "2nd"
    document.getElementById('class-name-for-subjects').textContent = `STD ${className}`;
    document.getElementById('subjects-medium-select').value = ''; // Reset medium dropdown
    document.getElementById('subjects-list-container').innerHTML = '<p class="text-center text-gray-500 col-span-full">Please select a medium to see subjects.</p>';
    document.getElementById('add-subject-btn').disabled = true; // Disable add button until medium is selected
...
function loadSubjects() {
    currentMedium = document.getElementById('subjects-medium-select').value;
    const addBtn = document.getElementById('add-subject-btn');
    const container = document.getElementById('subjects-list-container');
    
    if (!currentMedium) {
        container.innerHTML = '<p class="text-center text-gray-500 col-span-full">Please select a medium to see subjects.</p>';
    ...
    // THIS IS THE CORRECTED PATH to match your student app
    const path = `subjects/${currentClass}/${currentMedium}`;
    ...
    database.ref(path).on('value', snapshot => {
        ...
        if (snapshot.exists()) {
            ...
            for (const subjectId in subjects) {
                ...
                const card = document.createElement('div');
                card.className = 'bg-white p-4 rounded-lg shadow-md flex flex-col justify-between border';
                card.innerHTML = `
                    <div>
                    ...
                    <div class="flex space-x-2 mt-4">
                        ...
                        <button onclick="openSubjectModal('${subjectId}', '${subject.name}', '${subject.icon}', '${subject.color}')" class="bg-yellow-500 text-white font-bold py-2 px-3 rounded-lg text-sm"><i class="fas fa-pencil-alt"></i></button>
...
function loadLessons() {
    ...
    // THIS IS THE CORRECTED PATH to match your student app
    const path = `lessons/${currentClass}/${currentMedium}/${currentSubjectId}`;
            ...
            for (const lessonId in lessons) {
                ...
                item.innerHTML = `
                    <div>
                        ...
                        <p class="text-sm text-gray-600">${lesson.isPaid ? 'Premium' : 'Free'} - ${lesson.qna ? lesson.qna.length : 0} questions</p>
                    </div>
...
// --- Subject Modal & Form ---
function openSubjectModal(id = null, name = '', icon = '', color = '#4A90E2') {
...
// --- Lesson Modal & Form (No changes here, but pathing for save/load is fixed) ---
let qnaCount = 0;
function openLessonModal(lessonId = null) {
    ...
    if (lessonId) {
        // THIS IS THE CORRECTED PATH to match your student app
        const path = `lessons/${currentClass}/${currentMedium}/${currentSubjectId}/${lessonId}`;
...
function saveLesson(event) {
    ...
    const freeContent = document.getElementById('free-content').value;
    ...
    document.querySelectorAll('.qna-pair').forEach(pair => {
        ...
        if (question && answer) qna.push({ question, answer });
    });

    const lessonData = { title, freeContent, fullContent, isPaid, qna };
    // THIS IS THE CORRECTED PATH to match your student app
    const path = `lessons/${currentClass}/${currentMedium}/${currentSubjectId}`;
    let ref = id ? database.ref(`${path}/${id}`) : database.ref(path).push();
    
    ref.set(lessonData)
        .then(() => closeLessonModal())
...
function addQnaPair(question = '', answer = '') {
    ...
    div.innerHTML = `<input type="text" placeholder="Question ${qnaCount}" value="${question}" class="qna-question w-full p-2 border rounded-lg" required><div class="flex"><input type="text" placeholder="Answer ${qnaCount}" value="${answer}" class="qna-answer w-full p-2 border rounded-lg" required><button type="button" onclick="this.parentElement.parentElement.remove()" class="ml-2 text-red-500 p-2"><i class="fas fa-trash"></i></button></div>`;
...
function deleteSubject(subjectId) {
    const path = `subjects/${currentClass}/${currentMedium}/${subjectId}`;
    database.ref(path).remove().catch(error => console.error("Deletion error:", error));
}
...
function setupEventListeners() {
    ...
    document.getElementById('subjects-medium-select').addEventListener('change', loadSubjects);
}
